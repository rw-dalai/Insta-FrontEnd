import { Component, Input } from '@angular/core';
import { Post } from '@insta/data/post';
import { JsonPipe } from '@angular/common';
import {
	MatCard,
	MatCardActions,
	MatCardContent,
	MatCardHeader,
	MatCardImage,
	MatCardSubtitle,
	MatCardTitle,
} from '@angular/material/card';
import { MatButton } from '@angular/material/button';
import { MatIcon } from '@angular/material/icon';
import { MatChip, MatChipSet } from '@angular/material/chips';
import { AuthImageDirective } from '@insta/data/auth';

@Component({
	selector: 'insta-timeline-overview-post',
	standalone: true,
	imports: [
		JsonPipe,
		MatCard,
		MatCardHeader,
		MatCardImage,
		MatCardContent,
		MatButton,
		MatCardActions,
		MatCardTitle,
		MatCardSubtitle,
		MatIcon,
		MatChipSet,
		MatChip,
		AuthImageDirective,
	],
	templateUrl: './timeline-overview-post.component.html',
	styleUrl: './timeline-overview-post.component.css',
})
export class TimelineOverviewPostComponent {
	// icon = "data:image/png;base64,"

	@Input() post!: Post;
}
